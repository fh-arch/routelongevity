...
export default function AdminDashboard({ authSession, onOpenBlog, onOpenEvents }: AdminDashboardProps) {
  ...
  const renderQueue = (
    ...
    <section className="rounded-2xl border border-brand-warm-sand/45 bg-white p-4 shadow-sm">
      <div className="flex items-center justify-between border-b border-[#E5EDE1] pb-3">
      ...
      <div className="mt-4 space-y-3">
        {items.length === 0 ? (
          ...
          <article key={String(item.id)} className="rounded-2xl border border-[#E5EDE1] bg-[#FAFAF8] p-4">
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-[#FAFAF8] p-4 md:p-8 max-w-7xl mx-auto w-full space-y-8 animate-in fade-in duration-300">
      ...
      {notice && (
        <div className="rounded-2xl border border-[#A6D26A]/35 bg-[#F1F7EA] p-4 text-sm font-bold text-[#5A9D62]">
          {notice}
        </div>
      )}
      ...
      {overview && activeTab === 'listings' && (
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-[420px_minmax(0,1fr)]">
          ...
          <section className="rounded-3xl border border-brand-warm-sand/45 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 overflow-hidden rounded-2xl border border-[#E5EDE1]">
              {overview.content.listings.map((listing) => (
                <div key={String(listing.id)} className="grid grid-cols-[minmax(0,1fr)_120px_90px] gap-3 border-b border-[#E5EDE1] p-3 text-xs last:border-0">
                  <div className="min-w-0">
        ...
        <section className="rounded-3xl border border-brand-warm-sand/45 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 space-y-3">
            {overview.content.users.map((user) => (
              <article key={String(user.id)} className="flex flex-col gap-3 rounded-2xl border border-[#E5EDE1] bg-[#FAFAF8] p-4 md:flex-row md:items-center md:justify-between">
                <div>
