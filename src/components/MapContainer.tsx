      ...
      return L.divIcon({
        html: `
          <div class="relative flex items-center justify-center transform hover:scale-115 transition-transform duration-200">
            ${isPremium ? `
              <span class="absolute inline-flex h-9 w-9 rounded-full bg-[#0e655c] opacity-25 animate-pulse"></span>
            ` : ''}
            <!-- Inner Pin -->
            <div class="w-7 h-7 rounded-full border-2 shadow-md flex items-center justify-center relative z-10" style="border-color: ${ringColor}; background-color: #f6fbf9;">
              <div class="w-4.5 h-4.5 rounded-full flex items-center justify-center" style="background-color: ${categoryColor}">
                ${isPremium ? `
                  <svg width="10" height="10" viewBox="0 0 24 24" fill="#f6fbf9" stroke="#f6fbf9" stroke-width="1">
    ...
    if (showDensityLayer) {
      ...
      if ((L as any).heatLayer) {
        heatLayerRef.current = (L as any).heatLayer(heatData, {
          ...
          maxZoom: 9,
          gradient: {
            0.1: '#086058', // soft teal
            0.4: '#0e7a70', // turquoise
            0.7: '#0e655c', // warm copper
  ...
  return (
    <div className="flex flex-col md:flex-row h-full min-h-0 w-full relative">
      ...
      <div className={`absolute md:relative inset-y-0 left-0 w-80 sm:w-96 bg-white border-r border-[#d8ebe6]/40 flex flex-col z-30 md:z-10 shrink-0 shadow-lg transition-transform duration-300 ${showMobileList ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <div className="p-4 border-b border-brand-warm-sand/30 bg-[#f6fbf9]/40 space-y-3">
          ...
          <div className="pt-2 px-1 flex items-center justify-between border-t border-[#d8ebe6]/30">
            <div className="flex items-center gap-2">
              <span className="relative flex h-2 w-2">
                <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${showDensityLayer ? 'bg-[#A72B2B]' : 'bg-[#086058]/40'}`}></span>
                <span className={`relative inline-flex rounded-full h-2 w-2 ${showDensityLayer ? 'bg-[#A72B2B]' : 'bg-[#086058]/50'}`}></span>
              </span>
              <div>
            ...
            <button
              ...
              className={`scale-90 relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                showDensityLayer ? 'bg-[#086058]' : 'bg-[#d8ebe6]/50'
              }`}
            >
              <span
          ...
          <button
            ...
            className={`px-3 py-1 text-[11px] font-semibold rounded-full transition-all shrink-0 cursor-pointer ${
              selectedCategory === 'all'
                ...
                : 'bg-[#f6fbf9] text-brand-deep-slate/70 hover:bg-brand-warm-sand/30'
            }`}
          >
            ...
            <button
              ...
              className={`px-2.5 py-1 text-[11px] font-semibold rounded-full border transition-all shrink-0 cursor-pointer flex items-center gap-1 ${
                selectedCategory === cat.key
                  ...
                  : 'bg-white text-brand-deep-slate/70 border-brand-warm-sand/60 hover:bg-[#f6fbf9]'
              }`}
            >
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4 bg-[#f6fbf9]/20">
          <div className="flex justify-between items-center text-xs text-brand-deep-slate/55 px-1">
            ...
            {countryFilter && (
              <span className="font-semibold text-[#086058]">
                {countryFilter}
              </span>
            )}
          </div>
                    ...
                    <div className="flex-1 min-w-0 py-0.5">
                      <div className="flex flex-wrap items-start justify-between gap-2.5">
                        <span className={`text-[9px] font-bold tracking-wider uppercase px-2 py-0.5 rounded-full ${
                          ...
                          p.category === 'mediterranean-diet' ? 'bg-[#79c9b8]/15 text-[#79c9b8]' :
      ...
      <div className="flex-1 h-full bg-[#f6fbf9] relative">
        <div ref={mapRef} className="w-full h-full z-0" />

        {/* Heatmap Legend */}
        {showDensityLayer && (
          <div className="absolute top-4 right-4 z-10 bg-white/95 backdrop-blur-sm rounded-2xl border border-[#d8ebe6]/60 p-4 shadow-xl max-w-[260px] sm:max-w-xs animate-in fade-in slide-in-from-top-2 duration-300 pointer-events-auto">
            ...
            <div className="flex items-center gap-2">
              <span className="text-[9px] font-mono text-[#042f2c]/40">{t('low')}</span>
              <div className="h-2 flex-1 rounded-full bg-gradient-to-r from-[#086058] via-[#0e7a70] via-[#0e655c] to-[#A72B2B] shadow-inner" />
        ...
        <button
          ...
          style={{ backgroundColor: '#042f2c' }}
        >
          {showMobileList ? <X className="w-5 h-5 text-[#0e7a70]" /> : <Filter className="w-5 h-5 text-[#0e7a70]" />}
        </button>
          ...
          <div className="absolute bottom-24 left-4 right-4 md:bottom-auto md:top-4 md:right-4 z-10 w-auto md:w-96 bg-white rounded-2xl border border-brand-warm-sand/50 shadow-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-4 md:slide-in-from-top-4 duration-300">
            <div className="relative h-44">
              ...
              <button
                ...
                className="absolute top-3 right-3 bg-[#f6fbf9] hover:bg-white text-brand-deep-slate p-1.5 rounded-full shadow-lg border border-brand-warm-sand/30 transition-all cursor-pointer z-20"
              >
              ...
              <button
                ...
                className="absolute top-3 left-3 bg-[#f6fbf9] hover:bg-white text-brand-deep-slate p-2 rounded-full shadow-lg border border-brand-warm-sand/30 transition-all cursor-pointer z-20"
              >
            ...
            <div className="p-4 space-y-3.5">
              <div className="flex justify-between items-center bg-[#f6fbf9] p-2.5 rounded-xl border border-brand-warm-sand/40">
