...
export default function AuthModal({ isOpen, initialRole, initialMode, onClose, onSuccess }: AuthModalProps) {
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-[70] bg-[#042f2c]/75 backdrop-blur-sm flex items-center justify-center p-4">
        <motion.div
          initial={{ opacity: 0, y: 18, scale: 0.98 }}
          animate={{ opacity: 1, y: 0, scale: 1 }}
          exit={{ opacity: 0, y: 18, scale: 0.98 }}
          transition={{ duration: 0.18 }}
          className="w-full max-w-4xl max-h-[92vh] bg-[#f6fbf9] border border-brand-warm-sand/60 rounded-2xl shadow-2xl overflow-y-auto grid grid-cols-1 md:grid-cols-[0.9fr_1.1fr]"
        >
          ...
          <section className="p-6 md:p-8 relative">
            ...
            {success ? (
              <div className="min-h-[320px] flex flex-col items-center justify-center text-center">
                <div className="w-16 h-16 rounded-full bg-[#79c9b8]/12 text-[#79c9b8] flex items-center justify-center mb-4">
